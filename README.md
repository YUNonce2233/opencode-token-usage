...
```
<!-- opencode-token-usage -->
[Token 统计] 本轮合计 62,625 tok · 输入 2,049 · 输出 1,696 · 缓存读 58,880 · 缓存命中率 96.6% · 1 次调用 · 首字 24.0s · 70.7 tok/s · 上下文 60,929/128,000(47.6%) ✓
```
...
| 首字时间 | 从消息 parts 时间戳计算 TTFT，显示 `首字 X.Xs`；会话/项目/全局展示平均值 |
| 生成速率 | 输出 token 数 ÷ 生成耗时，显示 `X.X tok/s`；聚合范围展示平均值 |
