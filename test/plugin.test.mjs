...
test("完成的主 agent 回复自动追加 badge 且去重", async () => {
  await runCase(async ({ hooks, calls, getText }) => {
    ...
    assert.ok(badge.includes("[Token 统计] 本轮合计"), "应含统计文本")
    assert.ok(badge.includes(" 1 次调用"), "应含调用次数")
    assert.ok(/✓\s*$/.test(badge.trim().replace(/`$/, "")), "行尾应有 ✓")
    assert.ok(/缓存读 [\d,]+ tok · 缓存命中率 96\.6%/.test(badge), "应含缓存与命中率")
    assert.ok(/首字 \d+\.\d+s/.test(badge), "应含首字时间")
